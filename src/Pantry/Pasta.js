...
import productOptions from "../Database/PastaImages.json";
...
const Pasta = () => {
        ...
        getDocs(q)
          .then((querySnapshot) => {
            if (querySnapshot.empty) {
              ...
              setPasta(data.pasta || []);
              setFetchedProducts(data.pasta || []); // Set fetched products in state
  ...
  const saveMeatProduct = async () => {
    ...
    if (
      fetchedProducts.some((product) => product.name === newPastaProduct.name)
    ) {
      ...
      try {
        ...
        setFetchedProducts(updatedPasta);
        ...
        fetchedProducts.map((product, index) => {
          if (
            fetchedProducts.some(
              (product) => product.name === newPastaProduct.name
            )
          ) {
        ...
        console.error("Error saving pasta product:", error);
        alert("Error saving pasta product:", error);
      }
    }
  };
  ...
  const deleteProduct = (productToBeDeleted) => {
    ...
    const updatedPasta = pasta.filter(
      (product) => product.name !== productToBeDeleted
    );
    const userRef = doc(db, "users", user.uid);

    updateDoc(userRef, { pasta: updatedPasta })
      .then(() => {
        console.log("Pasta product deleted successfully!");
      ...
      .catch((error) => {
        console.error("Error deleting pasta product:", error);
        alert("Error deleting pasta product:", error);
      });
  };

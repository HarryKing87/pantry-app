...
const Vegetable = () => {
  ...
  const saveVegetableProduct = async () => {
    ...
    if (
      ...
      const userRef = doc(db, "users", user.uid);
      const updatedVegetable = [...vegetable, newVegetableProduct]; // Create a new array with the added product

      try {
        await updateDoc(userRef, { vegetables: updatedVegetable });

        // Update the state of the products list directly
        setVegetable(updatedVegetable);
        setFetchedProducts(updatedVegetable);
  ...
  const deleteProduct = (productToBeDeleted) => {
    ...
    const updatedVegetable = vegetable.filter(
      (product) => product.name !== productToBeDeleted
    );
    const userRef = doc(db, "users", user.uid);

    updateDoc(userRef, { vegetables: updatedVegetable })
      .then(() => {
  ...
  return (
    <div className="dashboard-dairy">
      ...
      ></script>
      <Navigation />
      <h3 style={{ margin: "2rem auto" }}>Vegetable section.</h3>

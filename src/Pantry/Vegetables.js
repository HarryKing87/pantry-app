...
import productOptions from "../Database/VegetableImages.json";
/* Dropdown Import */
...
const Vegetables = () => {
        ...
        getDocs(q)
          .then((querySnapshot) => {
            if (querySnapshot.empty) {
              ...
              setFetchedProducts(data.vegetables || []); // Set fetched products in state
            }
          })
          .catch((error) => {
  ...
  const saveVegetableProduct = async () => {
    const newVegetableProduct = {
      name: productName,
    ...
    if (
      fetchedProducts.some(
        (product) => product.name === newVegetableProduct.name
      )
    ) {
      alert("The product you selected already exists in your storage.");
    } else {
      const userRef = doc(db, "users", user.uid);
      const updatedVegetables = [...vegetables, newVegetableProduct]; // Create a new array with the added product

      try {
        await updateDoc(userRef, { vegetables: updatedVegetables });

        // Update the state of the products list directly
        setVegetables(updatedVegetables);
        setFetchedProducts(updatedVegetables);
        ...
        console.error("Error saving vegetable product:", error);
        alert("Error saving vegetable product:", error);
      }
    }
  };
  ...
  const deleteProduct = (productToBeDeleted) => {
    ...
    const updatedVegetables = vegetables.filter(
      (product) => product.name !== productToBeDeleted
    );
    const userRef = doc(db, "users", user.uid);

    updateDoc(userRef, { vegetables: updatedVegetables })
      .then(() => {
        console.log("Vegetable product deleted successfully!");
      ...
      .catch((error) => {
        console.error("Error deleting vegetable product:", error);
        alert("Error deleting vegetable product:", error);
      });
  };
  ...
  return (
    <div className="dashboard-dairy">
      ...
      ></script>
      <Navigation />
      <h3>Vegetables section.</h3>
      <form className="dairy-form">
        <Dropdown
          value={productName}
          options={productOptions}
          onChange={handleProductNameChange}
          placeholder="Select Vegetable Product"

...
function EditProfile() {
  ...
  const updateUserData = async (userRef, dataToUpdate) => {
    try {
      ...
      if (dataToUpdate.backgroundImage) {
        await updateDoc(userRef, { backgroundImage: dataToUpdate.backgroundImage });
      }
    } catch (error) {
  ...
  const handleImageChange = (event) => {
    ...
    if (file) {
      ...
      reader.onload = () => {
        setBackgroundImage(reader.result); // Store the data URL in the state
      };
  ...
  return (
    <div>
      <form className="edit-profile-container">
        <i style={{fontSize: "9px"}}>Recommended dimensions: 2560 x 1440 </i>
